...
public class GestoreAule {
    ...
    public Set<Aula> cercaAuleLibere(Set<Facility> requestedFacilities, TimeSlot ts) {
        if (requestedFacilities == null || ts == null) {
            throw new NullPointerException("Almeno uno dei parametri passati è nullo.");
        }

        Iterator<Aula> aulaIterator = aule.iterator();
        Set requested = new HashSet<Aula>();
        while (aulaIterator.hasNext()) {
            Aula current = aulaIterator.next();
            if (current.isFree(ts) && current.satisfiesFacilities(requestedFacilities)) {
